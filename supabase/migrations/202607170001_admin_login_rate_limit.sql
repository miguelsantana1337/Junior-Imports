...
alter table public.storefront_rate_limits
  ...
  check (action in ('order', 'coupon', 'password_reset', 'password_verify', 'login'));
...
begin
  if p_action not in ('order', 'coupon', 'password_reset', 'password_verify', 'login') then
...
revoke all on function public.clear_storefront_rate_limit(uuid, text, text)
  from public, anon, authenticated;
grant execute on function public.clear_storefront_rate_limit(uuid, text, text)
  to service_role;
