...
alter table public.storefront_rate_limits
  ...
  check (action in ('order', 'coupon', 'cart', 'funnel_event', 'password_reset', 'password_verify', 'login'));
...
begin
  if p_action not in ('order', 'coupon', 'cart', 'funnel_event', 'password_reset', 'password_verify', 'login') then
  ...
  if p_limit < 1 or p_limit > 300 or p_window_seconds < 10 or p_window_seconds > 86400 then raise exception 'Configuração de limite inválida'; end if;
...
as $$
begin
  if p_action not in ('order', 'coupon', 'cart', 'funnel_event', 'password_reset', 'password_verify', 'login') then raise exception 'Ação de segurança inválida'; end if;
  if length(trim(coalesce(p_fingerprint_hash, ''))) < 16 then raise exception 'Identificador de segurança inválido'; end if;
