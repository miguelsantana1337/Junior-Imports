...
export const settingsSchema = z.object({
  ...
  promotionName: z.string().trim().min(3).max(120),
  promotionStartsAt: z.union([z.literal(""), z.string().datetime({ offset: true })]),
  promotionEndsAt: z.union([z.literal(""), z.string().datetime({ offset: true })]),
  ...
  cardInstallmentMinimum: money,
  loyaltyDiscountEnabled: z.boolean(),
  loyaltyOrderInterval: z.coerce.number().int().min(2).max(100),
