...
export function ProductArt({ product, large = false }: { product: StorefrontProduct; large?: boolean }) {
  ...
  if (product.imageUrl && failedImageUrl !== product.imageUrl) {
    ...
    return (
      ...
      <img className="product-image product-image-framed" style={framingStyle} src={product.imageUrl} alt={product.name} onError={() => setFailedImageUrl(product.imageUrl)} />
    );
  }
