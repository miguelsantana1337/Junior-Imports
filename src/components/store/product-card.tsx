  ...
  return (
    <article className="product-card" data-testid={`product-${product.slug}`}>
      <div className="product-card-visual">
        ...
        <Link href={`/produtos/${product.slug}`} aria-label={`Ver detalhes de ${product.name}`}><ProductArt product={product} /></Link>
        <Link className="quick-link" href={`/produtos/${product.slug}`}>Ver detalhes</Link>
      </div>
      <div className="product-card-body">
        <div className="product-meta"><span>{product.category}</span><small className={`stock-${stock.tone}`}>{stock.label}</small></div>
        <Link href={`/produtos/${product.slug}`}><h3>{product.name}</h3></Link>
