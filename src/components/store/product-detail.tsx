      ...
      <section className="product-page container">
        ...
        <div className="product-detail-grid">
          ...
          <div className="product-detail-copy">
            ...
            <div className="detail-price price-stack">{product.compareAt > product.price && <del>{formatMoney(product.compareAt)}</del>}<strong>{formatMoney(product.price)}</strong>{isPixDiscountEligible(data.settings, product.price) && <small>{data.settings.pixDiscount}% OFF no Pix</small>}</div>
            ...
            <dl className="product-facts"><div><dt>Marca</dt><dd>{product.brand || data.settings.storeName}</dd></div><div><dt>Disponibilidade</dt><dd className={`stock-${stock.tone}`}>{stock.label}</dd></div><div><dt>Pedido</dt><dd>{cartEligible ? "Disponível para pedido" : product.stock <= 0 ? "Indisponível" : "Fale com a equipe"}</dd></div><div><dt>Entrega</dt><dd>{data.settings.shippingCityRates.length ? "Frete calculado pela cidade do CEP" : data.settings.freeShippingEnabled && isStorePromotionRuleActive(data.settings) ? `Frete grátis acima de ${formatMoney(data.settings.freeShippingThreshold)}` : `Frete fixo de ${formatMoney(data.settings.shippingFlat)}`}</dd></div></dl>
