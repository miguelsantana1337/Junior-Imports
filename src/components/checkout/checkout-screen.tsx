  ...
  async function submit(values: CheckoutInput) {
    ...
    if (!demoMode) {
      ...
      if (result.error || !result.data) {
        setSubmitError(result.error?.message ?? "Não foi possível registrar o pedido demonstrativo.");
        return;
      }
      persisted = result.data as PersistedOrder;
    }

    const code = persisted?.code ?? `JI-${nextNumber}`;
    const order: Order = {
    ...
    addOrder(order);
    clearCart();
    router.push(`/pedidos/${code}`);
  }

  if (!lines.length) {
    return <section className="page-state container"><span className="section-kicker">CHECKOUT DEMONSTRATIVO</span><h1>Seu carrinho está vazio.</h1><p>Adicione ao menos um produto antes de iniciar a simulação.</p><Link className="button button-primary" href="/#catalogo">Ver produtos</Link></section>;
  }

  return (
    <section className="checkout-page container">
      <Link className="back-link" href="/"><ArrowLeft /> Continuar comprando</Link>
      <div className="checkout-page-heading"><span className="section-kicker">CHECKOUT DEMONSTRATIVO</span><h1>Finalize seu pedido.</h1><p>Nenhuma cobrança será realizada. Este fluxo existe apenas para demonstração.</p></div>
      <div className="checkout-grid">
        <form className="checkout-form" onSubmit={handleSubmit(submit)} noValidate>
          ...
          <label className="consent-line"><input type="checkbox" {...register("consent")} /><span>Concordo que esta é uma simulação e nenhum pagamento será realizado.</span></label>{errors.consent && <small className="field-error">{errors.consent.message}</small>}
          {submitError && <p className="field-error" role="alert">{submitError}</p>}
          <button className="button button-primary button-full button-large" type="submit" disabled={isSubmitting}><LockKeyhole /> Criar pedido demonstrativo</button>
        </form>
        <aside className="checkout-summary"><span>RESUMO DO PEDIDO</span>{cartProducts.map(({ line, product }) => <div className="summary-item" key={line.productId}><i>JI</i><div><strong>{product!.name}</strong><small>{line.quantity} unidade{line.quantity > 1 ? "s" : ""}</small></div><b>{formatMoney(product!.price * line.quantity)}</b></div>)}<div className="summary-totals"><div><span>Subtotal</span><strong>{formatMoney(calculation.subtotal)}</strong></div><div><span>Descontos</span><strong>- {formatMoney(calculation.discount)}</strong></div><div><span>Frete</span><strong>{calculation.shipping ? formatMoney(calculation.shipping) : "Grátis"}</strong></div><div className="grand-total"><span>Total</span><strong>{formatMoney(calculation.total)}</strong></div></div><p className="summary-demo"><CheckCircle2 /> Valores apenas demonstrativos.</p></aside>
      </div>
    </section>
  );
}
