...
export function AutomationStudio() {
  ...
  return <section className="automation-studio-shell">
    ...
    <div className="automation-rule-list">{automations.map((automation) => <article key={automation.id} className={automation.status}>
      ...
      <div className="automation-flow-line"><span><Route /> {triggerLabels[automation.triggerType]}<b>{automation.triggerValue}</b></span><ArrowRight /><span><Workflow /> Condições<b>{automation.conditions.minOrderTotal > 0 ? `Pedido ≥ R$ ${automation.conditions.minOrderTotal}` : "Sem valor mínimo"}</b></span><ArrowRight /><span><Send /> Ações<b>{[automation.actions.sendMessage ? "Mensagem" : "", automation.actions.addTag ? "Etiqueta" : ""].filter(Boolean).join(" + ")}</b></span></div>
...
function AutomationBuilder({ automation, onClose }: { automation: MessageAutomation; onClose: () => void }) {
  ...
  async function persistAndMaybeTest(test: boolean) {
    const candidate = { ...form, actions: { ...form.actions, createTask: false, taskTitle: "" }, active: form.status === "active" };
