    ...
    <>
      ...
      <AdminPanel title="Backup e manutenção" description="Proteja a base remota e mantenha exportações auxiliares do painel.">
        <div className="data-actions">
          ...
          <article><Trash2 /><h3>Limpar pedidos</h3><p>{demoMode ? "Apaga somente os pedidos demonstrativos registrados." : "Remove pedidos e registros de mensagens da base ativa. Use somente em uma manutenção autorizada."}</p><button className="admin-button danger" onClick={async () => { const accepted = await confirm({ title: "Limpar pedidos?", description: demoMode ? "Todos os pedidos e registros de mensagens demonstrativos serão removidos." : "ATENÇÃO: todos os pedidos e registros de mensagens da operação ativa serão removidos.", confirmLabel: "Limpar pedidos", danger: true }); if (accepted) await clearOrders(); }}>Limpar pedidos</button></article>
